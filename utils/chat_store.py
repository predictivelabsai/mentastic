    ...
    with pool.get_session() as session:
        if title is not None:
            session.execute(text("""
                INSERT INTO mentastic.chat_conversations (thread_id, user_id, title)
...
def save_message(thread_id: str, role: str, content: str,
    ...
    with pool.get_session() as session:
        session.execute(text("""
            INSERT INTO mentastic.chat_messages (thread_id, message_id, role, content, metadata)
...
def delete_conversation(thread_id: str):
    ...
    with pool.get_session() as session:
        session.execute(text("""
            DELETE FROM mentastic.chat_conversations WHERE thread_id = :tid
        """), {"tid": thread_id})
